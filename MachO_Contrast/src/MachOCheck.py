...
def checkDifferences(pathOne, pathTwo):
    ...
    seq = difflib.SequenceMatcher(None, conteOne, conteTwo)
    ratio = seq.quick_ratio()
    print('相似度：', ratio)
    ##patches = dmp.diff_main(conteOne, conteTwo)

    #patches2 = dmp.patch_make(conteTwo, patches)
    #diff = dmp.patch_toText(patches)
    #ratio_two = dmp.patch_make(conteTwo, patches)
    #ratio_two = patches[0][0][2][1]

    #sim = Levenshtein.ratio('22abcd', '2abd1')
    #print('\ndiff_match_patch：', sim)
    return ratio
